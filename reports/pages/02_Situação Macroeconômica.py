...
with tab_credito_logistica:
    ...
    st.plotly_chart(
        generate_graphs._register_business_top10(get_data.DF_WBPY(years_to_filter))
        , use_container_width=True
    )

with tab_logistica_pop:
    st.plotly_chart(
        generate_graphs._crescimento_pop_top10(get_data.DF_WBPY(years_to_filter))
        , use_container_width=True
    )
    ...
    with col1:
        st.plotly_chart(
            generate_graphs._logistic_groups(get_data.DF_WBPY(years_to_filter))
            , use_container_width=True
        )
    
    with col2:
        st.plotly_chart(
            generate_graphs._logistic_bests(get_data.DF_WBPY(years_to_filter))
            , use_container_width=True
        )
